defmodule App.Chat.ChatReminder do
  ...
  @doc """
  Creates a changeset for chat reminder creation.
  """
  def create_changeset(reminder, attrs) do
    reminder
    |> cast(attrs, [:user_id, :treaty_id, :title, :description, :scheduled_at, :notification_type, :recurring_type, :priority])
    |> validate_required([:user_id, :treaty_id, :title, :scheduled_at])
  ...
  @doc """
  Creates a changeset for chat reminder updates.
  """
  ...
  @doc """
  Creates a changeset for marking as pending.
  """
  def mark_pending_changeset(reminder) do
    change(reminder,
      status: "pending",
      completed_at: nil
    )
