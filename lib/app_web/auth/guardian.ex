...
"""
  ...
  @impl true
  def after_decode_and_verify(claims, token, _options) do
    case resource_from_claims(claims) do
      {:ok, resource} -> {:ok, %{token: token, resource: resource, claims: claims}}
      {:error, reason} -> {:error, reason}
    end
  end

  @impl true
  def after_verify(_claims, _token, _options) do
    {:ok, :verified}
  end
end
