defmodule AppWeb.ChatLive do
  ...
  def handle_event("send_message", %{"message" => text}, socket) do
    ...
    Logger.info(
      "DEBUG: Evento send_message recebido. Texto: #{inspect(text)}. Uploads: #{inspect(socket.assigns.uploads.image.entries)}"
    )
    ...
    cond do
      ...
      true ->
        ...
        case App.Chat.send_message(
          ...
          {:ok, _message} ->
            Logger.info(
              "Message sent by #{socket.assigns.current_user} in order #{socket.assigns.order_id}"
            )
