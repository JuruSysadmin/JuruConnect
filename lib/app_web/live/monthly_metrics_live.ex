defmodule AppWeb.MonthlyMetricsLive do
  ...
  def render(assigns) do
    ...
    <div class="w-full flex min-w-0">
      <div class="card bg-base-100/80 backdrop-blur-xl shadow-lg border border-base-300/50 p-4 sm:p-5 transition-all duration-300 hover:shadow-xl w-full h-full flex flex-col min-w-0">
        <div class="text-center mb-2">
          <h2 class="card-title text-xs sm:text-sm font-bold text-primary mb-1 flex items-center justify-center gap-1 truncate">
        ...
        <div class="flex flex-col items-center gap-2 flex-1 justify-between min-h-0">
          <!-- Gráfico Circular -->
            ...
            <% else %>
              ...
              <div class="text-center">
                <div class="text-xs sm:text-sm font-medium text-base-content">
                  <span class="text-primary font-semibold">{@sale_mensal}</span>
          ...
          <!-- Cards horizontais -->
          <div class="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3 w-full">
            <div class="bg-base-200/80 backdrop-blur-sm rounded-lg p-3 sm:p-4 flex flex-col items-center shadow-sm border border-base-300/50 overflow-hidden">
              ...
              <div class="font-mono text-sm font-bold text-base-content text-center truncate w-full">{@objetivo_mensal}</div>
            </div>

            <div class="bg-primary/10 backdrop-blur-sm rounded-lg p-3 sm:p-4 flex flex-col items-center shadow-sm border border-primary/20 overflow-hidden">
              <div class="flex items-center gap-1.5 text-xs sm:text-sm text-primary mb-1.5">
                <svg xmlns='http://www.w3.org/2000/svg' class='h-2.5 w-2.5 sm:h-3 sm:w-3' fill='none' viewBox='0 0 24 24' stroke='currentColor' aria-hidden="true">
                  <path stroke-linecap='round' stroke-linejoin='round' stroke-width='2' d='M3 17l6-6 4 4 8-8' />
                </svg>
                Vendas Mensal
              </div>
              <div class="font-mono text-sm font-bold text-primary text-center truncate w-full">{@sale_mensal}</div>
            </div>

            <div class="bg-secondary/10 backdrop-blur-sm rounded-lg p-3 sm:p-4 flex flex-col items-center shadow-sm border border-secondary/20 overflow-hidden">
              <div class="flex items-center gap-1.5 text-xs sm:text-sm text-secondary mb-1.5">
                <svg xmlns='http://www.w3.org/2000/svg' class='h-2.5 w-2.5 sm:h-3 sm:w-3' fill='none' viewBox='0 0 24 24' stroke='currentColor' aria-hidden="true">
                  <path stroke-linecap='round' stroke-linejoin='round' stroke-width='2' d='M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z' />
                </svg>
                Ticket Médio Mensal
              </div>
              <div class="font-mono text-sm font-bold text-secondary text-center truncate w-full">{@ticket_medio_mensal}</div>
            </div>

            <%= if @show_goal_remaining do %>
              <%= if @goal_exceeded do %>
                <div class="bg-success/10 backdrop-blur-sm rounded-lg p-3 sm:p-4 flex flex-col items-center shadow-sm border border-success/20 overflow-hidden">
                  <div class="flex items-center gap-1.5 text-xs sm:text-sm text-success mb-1.5">
                    <svg xmlns='http://www.w3.org/2000/svg' class='h-2.5 w-2.5 sm:h-3 sm:w-3' fill='none' viewBox='0 0 24 24' stroke='currentColor' aria-hidden="true">
                      <path stroke-linecap='round' stroke-linejoin='round' stroke-width='2' d='M13 7h8m0 0v8m0-8l-8 8-4-4-6 6' />
                    </svg>
                    Excedente
                  </div>
                  <div class="font-mono text-sm font-bold text-success text-center truncate w-full">+{@goal_remaining_display}</div>
                </div>
              <% else %>
                <div class="bg-warning/10 backdrop-blur-sm rounded-lg p-3 sm:p-4 flex flex-col items-center shadow-sm border border-warning/20 overflow-hidden">
                  <div class="flex items-center gap-1.5 text-xs sm:text-sm text-warning mb-1.5">
                    <svg xmlns='http://www.w3.org/2000/svg' class='h-2.5 w-2.5 sm:h-3 sm:w-3' fill='none' viewBox='0 0 24 24' stroke='currentColor' aria-hidden="true">
                      <path stroke-linecap='round' stroke-linejoin='round' stroke-width='2' d='M12 8v4l3 3' />
                    </svg>
                    Falta
                  </div>
                  <div class="font-mono text-sm font-bold text-warning text-center truncate w-full">{@goal_remaining_display}</div>
                </div>
              <% end %>
            <% else %>
              <div class="bg-base-200/80 backdrop-blur-sm rounded-lg p-3 sm:p-4 flex flex-col items-center shadow-sm border border-base-300/50 overflow-hidden">
                ...
                <div class="font-mono text-sm font-bold text-base-content text-center truncate w-full">N/A</div>
              </div>
            <% end %>
          </div>
        </div>
